...
function buildSystemInstruction(
  ...
  blocks.push(
    [
      ...
      `[등장 인물]의 "말투" 항목에 실제 말버릇 예시 문장(어미, 즐겨 쓰는 표현 등)이 적혀 있다면, 그 문장의 분위기만 흉내 내지 말고 그 어미·어휘·문장 습관을 최대한 그대로 재현한다. 대사 하나하나가 그 인물의 예시 문장이 진짜로 이어지는 것처럼 읽혀야 한다.`,
    ].join("\n")
  );
  ...
  blocks.push(
    [
      ...
      `각 인물의 행동·대사를 정할 때는 직전 흐름의 관성보다 위 [등장 인물] 항목의 성격·말투를 매번 다시 기준으로 삼는다. 특히 앞선 화들의 전문을 읽고 나면 그 화들에서 굳어진 말투를 무의식적으로 따라가기 쉬운데, 그것이 [등장 인물]에 적힌 원래 말투와 조금이라도 달라졌다면 앞선 화가 아니라 반드시 원래 설정 쪽으로 되돌아온다.`,
...
function buildUserText(
  ...
  if (recentFull.length > 0) {
    blocks.push(
      ``,
      `방금 읽은 전문들의 말투가 아니라, 시스템 지시에 있는 [등장 인물]의 말투 설정(예시 문장이 있다면 그 표현)을 기준으로 대사를 써줘.`
    );
  }
