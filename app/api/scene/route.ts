...
function buildSystemInstruction(
  ...
  blocks.push(
    [
      ...
      `인물의 성격·말투(자주 쓰는 어미·화법)는 앞선 화들의 흐름과 무관하게 위 [등장 인물] 항목에 적힌 원래 설정을 매번 기준으로 삼는다. 특히 앞선 화들의 전문을 읽고 나면 그 화들에서 굳어진 말투를 무의식적으로 따라가기 쉬운데, 그것이 원래 설정과 조금이라도 달라졌다면 반드시 원래 성격·말투 쪽으로 되돌아온다.`,
      `단, 인물의 감정 상태·서로에 대한 태도·관계는 성격·말투와 다르게 이야기가 진행되며 실제로 변할 수 있다. [지난 구간 요약]·[지금까지의 줄거리]·[최근 화 전문]에서 이미 일어난 사건들이 쌓아온 감정·관계 변화는 그대로 유지하며 계속 발전시킨다. 화가 거듭될수록 인물의 감정이나 관계를 이야기 시작 시점의 기본 상태로 되돌리지 않는다.`,
