...
function buildSystemInstruction(
  ...
  blocks.push(
    [
      ...
      twoPart === "first"
        ? `사용자가 [다음 화 지시]를 줬고 이번 화가 그 사건을 두 화로 나눈 것 중 앞부분이라면, 아래 [다음 화 지시]에는 전체 사건 중 앞부분 내용만 나와 있다(뒷부분은 다음 화 몫이라 여기 안 보인다). 분량 제한(2800~3000자)을 채우려고 아직 나오지도 않은 뒷부분 사건을 미리 앞당겨 쓰거나 요약해서 언급하지 않는다 — 서두르지 말고 지금 주어진 내용만으로 밀도 있는 장면 묘사를 충분히 채운다.`
        : twoPart === "second"
          ? `사용자가 [다음 화 지시]를 줬고 이번 화가 그 사건을 두 화로 나눈 것 중 뒷부분이라면, 아래 [다음 화 지시]에는 전체 사건 중 뒷부분 내용만 나와 있다. 그 내용을 서두르지 말고 밀도 있는 장면 묘사로 충분히 그려 마저 완결한다.`
