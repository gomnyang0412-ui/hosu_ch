...
interface SceneRequestBody {
  ...
  directive?: string;
  /** "여러 화로 나눠 쓰기" 체크 시, 지금 요청이 그 사건의 첫 부분(first)·
   *  중간 부분(middle)·마지막 부분(last) 중 어디인지. 없으면 평범한
   *  단일 화 요청 — [규칙]의 "사건이 이번 화 안에서 분명히 일어나게"
   *  지시를 그대로 적용한다. */
  directiveSplit?: "first" | "middle" | "last";
  /** "+ 시간 경과" 입력으로 이번 화에 새로 추가할 일수(이미 환산됨). 여러
   *  화로 나눠 쓸 때는 첫 화 요청에만 실어 보낸다 — 서버가 저장한 값을
   *  다음 화 요청들이 다시 읽어오므로 중복으로 더할 필요가 없다. */
...
function buildSystemInstruction(
  ...
  blocks.push(observationRelationshipRule(universe));
  ...
  blocks.push(
    [
      ...
      directiveSplit === "first"
        ? `사용자가 [다음 화 지시]를 줬고 이번 화가 그 사건을 여러 화로 나눈 것 중 첫 부분이라면, 아래 [다음 화 지시]에는 전체 사건 중 첫 부분 내용만 나와 있다(나머지는 이후 화들 몫이라 여기 안 보인다). 분량 제한(2800~3000자)을 채우려고 아직 나오지도 않은 뒷부분 사건을 미리 앞당겨 쓰거나 요약해서 언급하지 않는다 — 서두르지 말고 지금 주어진 내용만으로 밀도 있는 장면 묘사를 충분히 채운다.`
        : directiveSplit === "middle"
          ? `사용자가 [다음 화 지시]를 줬고 이번 화가 그 사건을 여러 화로 나눈 것 중 중간 부분이라면, 아래 [다음 화 지시]에는 전체 사건 중 이번 화 몫의 내용만 나와 있다(앞부분은 이전 화들에서 이미 다뤘고, 뒷부분은 이후 화들 몫이라 여기 안 보인다). 분량 제한을 채우려고 앞뒤 내용을 앞당기거나 요약해서 언급하지 않는다 — 서두르지 말고 지금 주어진 내용만으로 밀도 있는 장면 묘사를 충분히 채운다.`
          : directiveSplit === "last"
            ? `사용자가 [다음 화 지시]를 줬고 이번 화가 그 사건을 여러 화로 나눈 것 중 마지막 부분이라면, 아래 [다음 화 지시]에는 전체 사건 중 마지막 부분 내용만 나와 있다. 그 내용을 서두르지 말고 밀도 있는 장면 묘사로 충분히 그려 마저 완결한다.`
      ...
      `장면에 필요한 사소한 배경·동작·감각 묘사는 설정에 없어도 각 인물의 성격과 세계관에 맞게 자연스럽게 채울 수 있다. 단, 인물 간 관계·공유된 과거·가족·연애·소속·신분은 이 자유 창작 범위에 포함되지 않으며, 위 [관계 기준]을 반드시 따른다.`,
    ].join("\n")
  );
