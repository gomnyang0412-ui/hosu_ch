...
interface SceneRequestBody {
  ...
  directive?: string;
  /** "2화로 나눠 쓰기" 체크 시, 지금 요청이 그 사건의 앞부분(first)인지
   *  뒷부분(second)인지. 없으면 평범한 단일 화 요청 — [규칙]의 "사건이
   *  이번 화 안에서 분명히 일어나게" 지시를 그대로 적용한다. */
...
function buildSystemInstruction(
  ...
  blocks.push(
    [
      ...
      twoPart === "first"
        ? `사용자가 [다음 화 지시]를 줬고 이번 화가 그 사건을 두 화로 나눈 것 중 앞부분이라면(아래 [다음 화 지시]에 안내가 있다), 지시문 내용을 이번 화 하나에 욱여넣어 급하게 다 보여주려 하지 않는다 — 분량 제한(2800~3000자)을 지키면서 지시문 속 사건의 앞부분만 느긋하게, 밀도 있는 장면 묘사로 충분히 그린다. 지시문에 사건이 여러 개 있거나 길더라도 전부를 압축해서 요약하듯 훑지 말고, 앞부분에 해당하는 내용만 깊게 다루고 나머지는 과감히 다음 화로 넘긴다. 사건의 결정적인 순간이나 결말은 이번 화에 넣지 않는다.`
        : twoPart === "second"
          ? `사용자가 [다음 화 지시]를 줬고 이번 화가 그 사건을 두 화로 나눈 것 중 뒷부분이라면(아래 [다음 화 지시]에 안내가 있다), 지시문 중 앞 화에서 아직 다루지 않은 나머지 부분을 이번 화에서 느긋하게, 밀도 있는 장면 묘사로 충분히 그려 마저 완결한다.`
